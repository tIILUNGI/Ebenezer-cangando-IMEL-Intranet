    ...
    <header className="h-16 md:h-20 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 md:px-8 flex items-center justify-between sticky top-0 z-30 shadow-sm transition-colors duration-300">
      ...
      <div className="flex items-center gap-2 sm:gap-4 md:gap-6">
        <div className="hidden xs:flex items-center gap-2">
          <button
            onClick={toggleLang}
            className="flex items-center gap-1.5 p-1.5 md:p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl text-slate-500 dark:text-slate-400"
          >
            <Languages size={18} />
          ...
          <button
            onClick={toggleTheme}
            className="p-1.5 md:p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl text-slate-500 dark:text-slate-400"
          >
          ...
          {showNotifs && (
            <div className="absolute right-0 mt-4 w-72 md:w-80 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-3xl shadow-2xl overflow-hidden animate-fade">
              <div className="p-4 md:p-6 bg-slate-50 dark:bg-slate-900/50 flex items-center justify-between border-b border-slate-100 dark:border-slate-700">
                ...
                <span className="text-[9px] bg-primary text-white px-2 py-0.5 rounded-full">
                  {unreadCount} Novas
                </span>
              </div>
              <div className="max-h-[300px] overflow-y-auto scrollbar-hide">
                {notifications.length === 0 ? (
                  <div className="p-10 text-center text-slate-300 italic text-sm font-bold">
                    Sem notificações
                  </div>
                ) : (
                  notifications.map((n) => (
                    ...
                    >
                      <div className="flex gap-3">
                        ...
                        {!n.read && (
                          <div className="w-1.5 h-1.5 bg-primary rounded-full mt-1 shrink-0"></div>
                        )}
                      </div>
                    </div>
                  ))
              ...
              <button
                ...
                className="w-full py-3 md:py-4 text-[9px] font-black uppercase tracking-widest text-primary bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors border-t border-slate-100 dark:border-slate-700"
              >
                Limpar tudo
              </button>
            </div>
