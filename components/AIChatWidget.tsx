...
const AIChatWidget: React.FC = () => {
  ...
  const currentGrades = useMemo(() => grades.filter(g => g.studentId === activeStudent?.id), [grades, activeStudent]);
  ...
  const askAiAboutSystem = async (e: React.FormEvent) => {
    ...
    const studentContext = user?.role === UserRole.ALUNO || user?.role === UserRole.ENCARREGADO
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role: 'ai', text: response.text || 'Sem resposta no momento.' }]);
    } catch (error) {
      setChatMessages(prev => [...prev, { role: 'ai', text: 'Não foi possível consultar a IA agora. Verifique a chave VITE_GEMINI_API_KEY.' }]);
    } finally {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-800 w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col">
            <div className="px-6 py-4 bg-primary text-white flex items-center justify-between">
              <h3 className="text-lg font-black flex items-center gap-2"><BrainCircuit size={18} /> Chat IA</h3>
              <button onClick={() => setChatOpen(false)} className="text-white font-black">Fechar</button>
            </div>
            <div className="p-6 space-y-3 max-h-[60vh] overflow-y-auto">
              {chatMessages.length === 0 ? (
                <p className="text-slate-400">Faça uma pergunta, por exemplo: "de acordo às minhas notas como estou".</p>
              ) : (
                chatMessages.map((m, idx) => (
                  <div key={idx} className={`p-4 rounded-2xl ${m.role === 'user' ? 'bg-blue-50 text-slate-800' : 'bg-emerald-50 text-slate-800'}`}>
                    <p className="text-xs font-black uppercase mb-1">{m.role === 'user' ? 'Você' : 'IA'}</p>
                    <p className="text-sm leading-relaxed">{m.text}</p>
                  </div>
                ))
              )}
            </div>
            <form onSubmit={askAiAboutSystem} className="p-4 border-t border-slate-100 dark:border-slate-700 flex gap-2">
              <input
                value={aiQuestion}
              ...
              />
              <button type="submit" disabled={isAiLoading} className="px-6 py-3 bg-primary text-white rounded-xl font-black text-xs uppercase tracking-widest disabled:opacity-60">
