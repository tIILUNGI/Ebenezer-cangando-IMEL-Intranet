...
const HelpPage: React.FC = () => {
  ...
  const filteredFAQs = AI_KNOWLEDGE_BASE.filter(
    (item) =>
    ...
    <div className="space-y-8 animate-fade max-w-4xl mx-auto">
      ...
      <div className="space-y-4">
        {filteredFAQs.map((faq, index) => (
          <div
            key={index}
            className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden transition-all"
          >
            <button
