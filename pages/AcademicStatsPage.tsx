...
import { fetchAcademicStats, fetchKPIs } from '../src/api/index';
import {
  BarChart,
  Bar,
  XAxis,
...
const AcademicStatsPage: React.FC = () => {
  ...
  const loadStats = async () => {
    ...
    try {
      const { data } = await fetchAcademicStats();
      setStats(data);
    } catch (err) {
  ...
  const handleExport = () => {
    const data = stats?.approvalData || [];
    const header = ['classe', 'aprovados', 'reprovados'];
    const rows = data.map((r) => [r.name, r.aprovados.toString(), r.reprovados.toString()]);
    ...
    Swal.fire({
      icon: 'success',
      title: 'Exportado!',
      text: 'Dados exportados como CSV.',
      timer: 1500,
    });
  };
  ...
  return (
    <div className="space-y-8 animate-fade">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Análise de desempenho por nível e curso.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-xl text-sm font-bold shadow-lg shadow-blue-900/10"
          >
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
          <h3 className="text-xl font-bold mb-8 flex items-center gap-2">
            <TrendingUp className="text-emerald-500" /> Aprovação vs Reprovação
          </h3>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats?.approvalData || []}>
                <CartesianGrid
                  strokeDasharray="3 3"
                  vertical={false}
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
          <h3 className="text-xl font-bold mb-8 flex items-center gap-2">
            <Award className="text-primary" /> Melhores Cursos (Ranking)
          </h3>
          <div className="space-y-6">
            {(stats?.topCourses || []).map((c, i) => (
              <div
                key={c.curso}
                className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl"
              >
      ...
      {stats?.declineAlert && (
        <div className="bg-primary/5 p-8 rounded-3xl border border-primary/20 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-slate-600 dark:text-slate-400">
                {stats.declineAlert.discipline}: há um declínio de desempenho comparado ao ano
                anterior.
              </p>
            </div>
          </div>
          <button className="px-6 py-3 bg-primary text-white rounded-xl font-bold hover:scale-105 transition-all">
            Ver Detalhes
          </button>
        </div>
      )}
